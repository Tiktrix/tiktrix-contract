...
/**
 * @title EthToMonadBridge
 * @dev Bridge contract for locking TRIX tokens on Ethereum (source chain)
 * and releasing on Monad (target chain).
 *
 * Flow:
 * 1. User calls lock(amount) on Ethereum - tokens locked in this contract
 * 2. Relayer monitors Lock events and calls release() on MonadToEthBridge (Monad side)
 *
 * Deployed on: Ethereum (Sepolia for testnet)
 */
contract EthToMonadBridge is PermissionsEnumerable, ContractMetadata, Multicall, ReentrancyGuard {
     ...
     * @param _trixToken Address of the TRIX token contract on Ethereum
     */
     ...
     * Users call this to initiate bridge from Ethereum to Monad
     ...
     * Called by relayer when tokens are locked on Monad (reverse direction)
     * @param user Address of the user to release tokens to
     * @param amount Amount of tokens to release
     * @param fromTxHash Transaction hash from Monad Lock transaction
     */
